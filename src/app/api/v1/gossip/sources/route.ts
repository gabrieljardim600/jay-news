...
const SELECT_COLS = `id, platform, handle, label, tier, active, last_fetched_at, last_post_count, last_error, created_at`;

export const GET = withService(async (_req, ctx) => {
  const supabase = accountClient(ctx);
  let q = supabase.from("gossip_sources").select(SELECT_COLS).order("created_at", { ascending: false });
  q = byAccount(q, ctx);
