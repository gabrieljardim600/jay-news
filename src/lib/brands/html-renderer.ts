...
/**
 * Gera um HTML de preview do design system — equivalente ao btg-whitelabel-cores.html
 * que o Gabriel curou manualmente. Mostra paleta organizada por role, tipografia e logos.
 */
export function renderDesignSystemHtml(ds: DesignSystem): string {
  ...
  const roleSwatches = [
    ds.colors.primary && swatch("Primary", ds.colors.primary.hex, "brand.primary"),
    ds.colors.secondary && swatch("Secondary", ds.colors.secondary.hex, "brand.secondary"),
    ds.colors.accent && swatch("Accent", ds.colors.accent.hex, "brand.accent"),
    ds.colors.background && swatch("Background", ds.colors.background.hex, "surface.default"),
    ds.colors.text && swatch("Text", ds.colors.text.hex, "content.primary"),
  ]
    .filter(Boolean)
    .join("\n");

  const neutralSwatches = ds.colors.neutral
    .map((n, i) => swatch(`Neutral ${i + 1}`, n.hex, `neutral.${i + 100}`))
    .join("\n");

  const logoHtml = ds.logos.variants.length
    ? `<div class="logos">${ds.logos.variants
        .map((url) => `<div class="logo-card"><img src="${escapeAttr(url)}" alt="logo"/></div>`)
        .join("")}</div>`
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .swatch-role { font-size: 10px; color: #444; margin-top: 2px; font-style: italic; }
...
<body>
  ...
  <h2>Typography</h2>
  <div class="typo">${typo || '<p class="muted">Nenhuma tipografia específica identificada.</p>'}</div>
...
function swatch(label: string, hex: string, role: string): string {
  return `<div class="swatch">
    ...
    <div class="swatch-info">
      ...
      <span class="swatch-role">${escapeHtml(role)}</span>
    </div>
  </div>`;
}
