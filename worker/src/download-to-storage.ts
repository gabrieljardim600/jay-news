...
import type { DownloadedAsset, RawAsset } from "./types.js";
...
export async function downloadAndUpload(
  ...
  try {
    ...
    const response = await fetch(asset.originalUrl, {
      ...
      redirect: "follow",
      headers: {
        "User-Agent": "Mozilla/5.0 (compatible; JayNewsWorker/1.0)",
        Accept: "*/*",
...
export async function uploadScreenshot(
  ...
  try {
    const slug = pageUrl.replace(/https?:\/\//, "").replace(/[^a-z0-9]/gi, "_").substring(0, 80);
    const storagePath = `${scrapeId}/screenshot/${slug}.png`;
    const { error } = await supabase.storage
      .from("brand-assets")
...
function extensionFor(url: string, contentType: string): string {
  ...
  const valid = ["png", "jpg", "jpeg", "webp", "gif", "svg", "ico", "avif", "woff", "woff2", "ttf", "otf"];
  if (fromUrl && valid.includes(fromUrl)) return fromUrl === "jpeg" ? "jpg" : fromUrl;
